...
**Goal:** The selected student architecture (TinyBERT-4) learns the teacher's decision boundary through intermediate layer transfer, achieving meaningfully higher F1 on the real-world holdout than the direct fine-tune baseline established in Phase 2.
...
**Success Criteria** (what must be TRUE):
  ...
  2. Layer mapping table between teacher (24 layers) and student (4 layers) is defined and documented before any training code is written; learnable linear projections handle dimension mismatch
